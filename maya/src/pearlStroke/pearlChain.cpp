...
void pearlChain::appendPointsTo(MPointArray &points, const MMatrix &wm) const
{
	std::vector<pearl>::const_iterator iter;

	if (wm.isEquivalent(MMatrix::identity))
	{

		for (iter = m_points.begin(); iter != m_points.end(); iter++)
		{
			points.append(MPoint(iter->point));
		}
		return;
	}
	...
	for (iter = m_points.begin(); iter != m_points.end(); iter++)
	{
		points.append(MPoint(iter->point) * wm);
	}
}

void pearlChain::appendPointsTo(MFloatPointArray &points, const MMatrix &wm) const
{
	std::vector<pearl>::const_iterator iter;
	...
	for (iter = m_points.begin(); iter != m_points.end(); iter++)
	{
		points.append(MFloatPoint(MPoint(iter->point) * wm));
	}
}
...
void pearlChain::appendRadiiTo(MFloatArray &radii, float scale) const
{
	std::vector<pearl>::const_iterator iter;

	for (iter = m_points.begin(); iter != m_points.end(); iter++)
	{
		radii.append(iter->radius * scale);
	}
}
