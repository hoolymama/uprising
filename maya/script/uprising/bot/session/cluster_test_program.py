...
class ClusterTestProgram(Program):
    def __init__(self, name, cluster, original_approach_height):

        super(ClusterTestProgram, self).__init__(name)
        self.cluster = cluster
        self.cache = {"index": None, "arrivals": None, "departure": None}

        self.height_reduction = (
            original_approach_height - (1 + (self.cluster.brush.active_bristle))
        ) * 10  # mm
    ...
    def send(self, **kw):
        ...
        self.cluster.send(self.program, frame)
